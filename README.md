...
```bash
cd ~
git clone https://github.com/<YOUR-ACCOUNT>/bio462_complabs.git
```

Then open the notebook for the day, e.g. `bio462_complabs/lab1_variant_interpretation/Lab1_Student.ipynb`,
select the kernel your instructor announces, and run from the top.
...
```bash
cd ~/bio462_complabs/lab3_singlecell
python data/download_dataset.py
```

or run the first notebook cell, which does the same thing. Do this during the pre-class buffer —
...
you do not need to install anything. (`env/install_esmfold.sh` is for the instructor.)
